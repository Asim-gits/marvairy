...
// Fix for price slider in ajax
	...
	if(current_user_can("wholesale_buyer")){
		...
		$sql .= " 	WHERE {$wpdb->posts}.post_type = 'product'
					...
					AND price_meta.meta_key IN ('" . implode("','", array_map('esc_sql', apply_filters('woocommerce_price_filter_meta_keys', array('wholesale_price')))) . "')
		...
		/*For Subscriber user*/
		$sql = "SELECT (MIN( price_meta.meta_value + 0.0 ) + (MIN( price_meta.meta_value + 0.0 ) * (19 / 100)))  AS min_price, (MAX( price_meta.meta_value + 0.0 ) + (MAX( price_meta.meta_value + 0.0 ) * (19 / 100))) AS max_price FROM {$wpdb->posts} ";
		$sql .= " LEFT JOIN {$wpdb->postmeta} as price_meta ON {$wpdb->posts}.ID = price_meta.post_id " . $tax_query_sql['join'] . $meta_query_sql['join'];
		$sql .= " 	WHERE {$wpdb->posts}.post_type = 'product'
					AND {$wpdb->posts}.post_status = 'publish'
	...
	if (version_compare(WOOCOMMERCE_VERSION, '2.6', '>')) {
	    $prices = self::get_filtered_price();
	    $min = ceil($prices->min_price);
	
	} else {
	    self::set_layered_nav_product_ids();
	    if (0 === sizeof(WC()->query->layered_nav_product_ids)) {
		$min = ceil($wpdb->get_var(
				$wpdb->prepare('
