...
class IgniteWoo_Wholesale_Pricing_Plus { 
	...
	function maybe_get_price_excluding_tax( $price, $qty, $_product ) { 
		...
		// Shop and product pages for EU users
			...
			if ( $_product->is_type( 'simple' ) )
				...
				$wprice = get_post_meta( $variation_id, 'wholesale_price', true );
			...
			if ( !empty( $wprice ) && 'yes' == get_option('woocommerce_prices_include_tax') ) {
				...
				if ( !empty( $variation_id ) )
					$this->without_vat_price[ $variation_id ] = $price; 
				else 
					$this->without_vat_price[ $product_id ] = $price; 
	...
	function maybe_get_price_html( $price, $_product ) {
		...
		if ( $_product->is_type( 'simple' ) )
			$wprice = get_post_meta( $product_id, 'wholesale_price', true );
		else if ( $_product->is_type( 'variation' ) || $_product->is_type( 'variable' ) )
			$wprice = get_post_meta( $variation_id, 'wholesale_price', true );
		...
		if ( 'eu' == $this->disable_tax && !empty( $this->without_vat_price[ $product_id ] ) && $this->without_vat_price[ $product_id ] > 0 ) {
		
			if ( function_exists( 'wc_price' ) ) { 

				$price = wc_price( round( $this->without_vat_price[ $product_id ], absint( get_option( 'woocommerce_price_num_decimals' ) ) ) );
			...
			if ( $_product->is_type( 'variable' ) )
				return '<span class="from">' . __( 'From:', 'woocommerce' ) . ' ' . wc_price( $wprice ) . ' </span>';
			else { 
	...
	function maybe_adjust_variations( $variation = '', $obj = '' , $variation_obj  = '') { 
		...
		if ( version_compare( WOOCOMMERCE_VERSION, '3.0', '>=' ) ) {
			...
			if ( !empty( $product_id ) ) 
				...
				$product_id = $variation_obj->get_id();
		} else {
			$product_id = $variation_obj->id;
		...
		if ( 'yes' == $this->disable_tax && 'yes' == get_option('woocommerce_prices_include_tax') ) { 
			...
			if ( !empty( $wprice ) ) {
				...
				if ( !empty( $_product->variation_id ) )
					$woocommerce_wholesale_pricing->without_vat_price[ $variation_id ] = $price; 
				else 
					$woocommerce_wholesale_pricing->without_vat_price[ $product_id ] = $price; 
			
				if ( function_exists( 'wc_price' ) )
					$vprice = wc_price( round( $price, get_option( 'woocommerce_price_num_decimals', 2 ) ) );
				else 
					$vprice = wc_price( round( $price, get_option( 'woocommerce_price_num_decimals', 2 ) ) );
			}
	...
	class WC_Wholesale_Skip_Payment_At_Checkout extends WC_Payment_Gateway {
		...
		function process_payment( $order_id ) {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.1', '>=' ) ) { 
				...
				return array(
					'result' 	=> 'success',
					'redirect'	=> add_query_arg( 'key', $order->order_key, add_query_arg( 'order', $order_id, get_permalink( wc_get_page_id( 'thanks' ) ) ) )
				);
			}
		}
