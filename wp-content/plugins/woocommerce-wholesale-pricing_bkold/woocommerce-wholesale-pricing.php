	...
	*/
	...
	function add_cart_item( $item_data = '' ) { 
		...
		if ( current_user_can( 'wholesale_buyer' ) ) {
			...
			//else if ( 'grouped' == $_product->product_type )
			//	$wholesale_price = ''; 
	...
	public static function maybe_return_wholesale_price( $price, $_product ) { 
		...
		if ( $_product->is_type('grouped') ) { 
			...
			$price = '<span class="from">' . __('From:', 'ignitewoo_wholesale') . ' </span>' . woocommerce_price( $min_price );

		} elseif ( $_product->is_type( $vtype ) ) {

			if ( version_compare( WOOCOMMERCE_VERSION, "2.0.0" ) >= 0 ) { 
				$wprice_min = get_post_meta( $_product->id, 'min_variation_wholesale_price', true );
				$wprice_max = get_post_meta( $_product->id, 'max_variation_wholesale_price', true );
			} else {
			...
			if ( current_user_can( 'wholesale_buyer' ) )  { 
				...
				if ( empty( $wprice_min ) && empty( $wprice_max ) ) { 
					...
					$wprice_min = get_post_meta( $_product->id, '_min_variation_price', true );
					$wprice_max = get_post_meta( $_product->id, '_min_variation__price', true );
				}
				...
				if ( $wprice_min == $wprice_max ) 
					return woocommerce_price( $wprice_min );
				
				else if ( isset( $wprice_min ) )
					//$price .= $this->format_price( $_product->product_custom_fields['min_variation_wholesale_price'][0] );
					$price = '<span class="from">' . __('From:', 'ignitewoo_wholesale') . ' ' . woocommerce_price( $wprice_min ) . ' </span>';;
			...
			if ( current_user_can( 'wholesale_buyer' ) )  { 
				...
				if ( version_compare( WOOCOMMERCE_VERSION, "2.0.0" ) >= 0 ) {
					$wprice_min = get_post_meta( $_product->id, 'wholesale_price', true );
				} else {
	...
	function is_purchasable( $purchasable, $_product ) { 
		...
		if ( $is_variation  ) { 
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.2', '>=' ) )
				$price = get_post_meta( $_product->id, 'min_variation_wholesale_price', true );
			else
			...
			// Simple products
			$price = $this->get_product_meta( $_product->id, 'wholesale_price', false );
	...
	function maybe_return_price( $price = '', $_product ) { 
		...
		if ( empty( $_product->variation_id ) && $_product->is_type( 'variable' ) ) {
		...
		} else if ( isset( $_product->variation_id ) ) {
	...
	function get_product_meta( $product, $field_name, $variation = false ) {
		...
		if ( version_compare( WOOCOMMERCE_VERSION, "2.0.0" ) >= 0 ) {
			...
			if ( $product->variation_id && isset( $product->product_custom_fields[ $prefix . $field_name ][0] ) && $product->product_custom_fields[ $prefix . $field_name ][0] !== '' ) {
				return $product->product_custom_fields[ $prefix . $field_name ][0];
			}

			if ( !$variation ) 
				return maybe_unserialize( get_post_meta( $product->id, $field_name, true ) );
			else
				return maybe_unserialize( get_post_meta( $product->variation_id, $field_name, true ) );
			...
			if ( isset( $product->variation_id ) && $product->variation_id ) {
				if ( ( $value = get_post_meta( $product->variation_id, $prefix . $field_name, true ) ) !== '' ) return $value;
				// otherwise return the value from the parent
				return get_post_meta( $product->id, $prefix . $field_name, true );
			}
	...
	function maybe_return_variation_price_empty( $price, $_product ) {
		...
		$min_variation_wholesale_price = get_post_meta( $_product->id, 'min_variation_wholesale_price', true );
		$max_variation_wholesale_price = get_post_meta( $_product->id, 'max_variation_wholesale_price', true );
		...
		if ( $min_variation_wholesale_price !== $max_variation_wholesale_price )
			$price = '<span class="from">' . __('From:', 'woocommerce') . ' ' .  woocommerce_price( $min_variation_wholesale_price ) . ' </span>';
		else 
	...
	function variation_attributes( $attrs, $product, $variation ) { 
		...
		$has_wholesale_price = get_post_meta( $variation->variation_id, 'wholesale_price', true );
		
		if ( $has_wholesale_price ) {

			$price = $this->maybe_return_price( $variation->price, $variation );

			$attrs['price_html'] = $variation->min_variation_price != $variation->max_variation_price ? '<span class="price"><ins>' . woocommerce_price( $price ) . '</ins></span>' : '';

		} else { 
		
			if ( $variation->is_on_sale() ) 
			
				$attrs['price_html'] = $variation->min_variation_price != $variation->max_variation_price ? '<span class="price"><del>' . $variation->regular_price . '</del> <ins>' . woocommerce_price( $variation->get_price() ) . '</ins></span>' : '';
		
			else 
			
				$attrs['price_html'] = $variation->min_variation_price != $variation->max_variation_price ? '<span class="price"><ins>' . woocommerce_price( $variation->get_price() ) . '</ins></span>' : '';
	...
	function maybe_return_variation_price( $price, $_product ) {
		...
		*/
		...
		if ( $is_variation && current_user_can( 'wholesale_buyer' ) ) { 
			...
			//$price = woocommerce_price( $_product->product_custom_fields['min_variation_wholesale_price'][0] );
			$w_price = get_post_meta( $_product->variation_id, 'wholesale_price', true );
			...
			if ( !empty( $ignitewoo_wholesale_pricing_plus ) && 'eu' == $ignitewoo_wholesale_pricing_plus->disable_tax && !empty( $w_price ) && 'yes' == get_option('woocommerce_prices_include_tax') ) {
				...
				return woocommerce_price( $w_price );
			}
			...
			//$price = woocommerce_price( get_post_meta( $_product->variation_id, 'wholesale_price', true ) );
		...
		if ( current_user_can( 'wholesale_buyer' ) )  { 
				...
				if ( is_array( $product->product_custom_fields['wholesale_price'] ) && $product->product_custom_fields['wholesale_price'][0] > 0 ) {
					...
					$price = woocommerce_price( $product->product_custom_fields['wholesale_price'][0] );
